...
class preprocessing():
    ...
    def slice_data2(self, x, ntrain, nval):
        return [x[:, :(ntrain+nval)],x[:, (ntrain+nval):]]
    ...
    def NewParams(self,x,norm=True,vectorial_norm=False):
        A = np.std(x,axis=0); B = np.mean(x,axis=0);
        if A.shape[0]>1: A[A==0]=1
        return torch.tensor(np.array([np.float64(A),np.float64(B)]))

    def GetParams_safe(self,x,norm=True,vectorial_norm=False, epsilon=1e-8):
        # 分离零值和非零值
        non_zero_mask = (x != 0)
        ...
        # 仅用非零部分计算参数
        if len(x_non_zero) > 1:
            mean = np.mean(x_non_zero)
            std = np.std(x_non_zero)
        else:
            mean, std = 0.0, 1e-8  # 全零列保护
    
        return torch.tensor(np.array([np.float64(mean),np.float64(std)]))
